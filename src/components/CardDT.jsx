...
const CardDT = () => {
  return (
    <div className="w-60 sm:w-96 mx-auto mt-2 mb-12 my-0 px-8 pt-4 pb-8 rounded-xl border border-b-4 border-gray-600 shadow-md bg-white">
      <div className="flex justify-between items-center">
        <h2 className="block mt-[-4px] text-2xl sm:text-3xl font-medium">
          DT <span className="block mt-[-4px] text-gray-400 text-xs sm:text-sm font-normal">Dolar Today</span>
        </h2>
        <img
          src={logoDT}
      ...
      <div className="w-56 sm:w-[23.5rem] relative left-[-2rem] ml-[-0.65rem] py-1 px-4 rounded-t-md rounded-b-md bg-yellow-300 ">
        <p className="m-0 pt-2 flex justify-end text-3xl font-medium before:content-['VES'] before:mr-1 before:font-light before:text-[14px] sm:before:text-[18px] after:content-['/_1$'] after:ml-1 after:font-light after:text-[14px] sm:after:text-[18px]">21.24</p>
        <div className="h-0 w-0 absolute bottom-[-20px] left-0 border-t-[13px] border-t-green-500 border-b-[10px] border-l-[13px] z-[-6]" ></div>
      </div>
      <div className="mt-4">
      ...
      <button className="flex justify-center items-center text-center mt-6 py-1 sm:py-2 px-4 sm:px-6 font-bold bg-[#84BA63] border border-b-transparent rounded-full pointer transition-all hover:bg-[#C2CD4B] active:scale-95">
        <a href="https://dolartoday.com/" target="_blank" className="font-medium text-lg">Ver mas</a>
